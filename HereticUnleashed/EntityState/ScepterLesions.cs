...
{
    class ScepterLesions : BaseSkillState
	{
		[SerializeField]
		...
		private void FireBleed(string muzzleString)
		{
			...
			if (NetworkServer.active)
			{
				...
				foreach (HurtBox hurtBox in enumerable)
				{
					...
					if (FriendlyFireManager.ShouldSplashHitProceed(hc, team))
					{
						if (willBleed)
						{
							...
							this.AddDebuff(body);
							body.AddTimedBuffAuthority(RoR2Content.Buffs.Slow80.buffIndex, bleedDuration);
				...
				if (willBleed)
				{
					AddDebuff(base.characterBody, selfBleedDamageMultiplier);
					base.characterBody.RecalculateStats();
				}
			}
		}
		private void AddDebuff(CharacterBody body, float damageMultiplier = 1)
		{
